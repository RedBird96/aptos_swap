module HoloCredits::holocredits {
    use std::error;
    use std::signer;
    use std::option;
    use std::string::utf8;
    ...
    const ASSET_SYMBOL: vector<u8> = b"HC";
    ...
    public fun get_metadata(): Object<Metadata> {
        let asset_address = object::create_object_address(&@HoloCredits, ASSET_SYMBOL);
    ...
    // borrow reference to metadata's refs (ManagedFungibleAsset)
    inline fun authorized_borrow_refs(
        owner: &signer,
        asset: Object<Metadata>,
    ): &ManagedFungibleAsset acquires ManagedFungibleAsset {
        // check that signer is the metadata object's owner
