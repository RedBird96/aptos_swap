module token_swap::swap {
    ...
    struct TokenPairMetadata<phantom X, phantom Y> has key {
        ...
        /// reserve_x * reserve_y as of last liquidity event
        k_last: u128,
        /// token 0 balance
    ...
    */
    ...
    public(friend) fun create_pair<X, Y>(sender: &signer) acquires SwapInfo {
        ...
        // create lp name
        let lp_name: string::String = string::utf8(b"Swap-");
        ...
        if (string::length(&lp_name) > MAX_COIN_NAME_LENGTH) {
            lp_name = string::utf8(b"Swap LPs");
        };
        ...
        // set up TokenPairReserve resource
        ...
        // set up TokenPairMetadata resource
        ...
        // set up PairEventHolder resource
    ...
    /// get LP token balance of addr
    /// only use to check other users' balances
    ...
    /// get current reserves of token 0 and token 1 with last updated timestamp
    ...
    /// get balance currently in pools for a pair
    ...
    */
    ...
    public(friend) fun add_liquidity<X, Y>(
        ...
        // add liquidity fn
        let (
            a_x, // amount x deposited (calculated to be optimal)
            a_y, // amount y deposited (calculated to be optimal)
            coin_lp,
            fee_amount,
            coin_left_x,
            coin_left_y
        ...
        let sender_addr = signer::address_of(sender);
        // check lp amount is > 0
        let lp_amount = coin::value(&coin_lp);
        assert!(lp_amount > 0, ERROR_INSUFFICIENT_LIQUIDITY);
        // deposit lp token and any remainders of token 0 and 1 to sender
    ...
    fun add_liquidity_direct<X, Y>(
        ...
        // get amount in reserves
        let (reserve_x, reserve_y, _) = token_reserves<X, Y>();
        // calculate optimal token 0 and token 1 to deposit
        ...
        // check that optimal amount to deposit is <= user provided amount
        ...
        deposit_x<X, Y>(x); // why are these x and y and not optimal amounts?
        deposit_y<X, Y>(y);
    ...
    public(friend) fun remove_liquidity<X, Y>(
        ...
        // withdraw amount of lp tokens from sender
        let coins = coin::withdraw<LPToken<X, Y>>(sender, liquidity);

        // calculate amount of token 0 and token 1 to remove
    ...
    */
    ...
    fun swap<X, Y>(
        ...
        // No need to use u256 when
        // balance_x_adjusted * balance_y_adjusted and 
